...
export default async function HomePage() {
  ...
  // Dynamically find sections by type instead of hardcoded indices
  const hero = s.find((sec: any) => sec?.type === 'hero');
  const intro = s.find((sec: any) => sec?.type === 'intro');
  const rooms = s.find((sec: any) => sec?.type === 'room_grid');
  const editorialSplits = s.filter((sec: any) => sec?.type === 'editorial_split');
  const dining = editorialSplits[0] ?? null;
  const family = editorialSplits[1] ?? null;
  const weddings = editorialSplits[2] ?? null;
  const wellness = s.find((sec: any) => sec?.type === 'wellness_scroll');
  const gallery = s.find((sec: any) => sec?.type === 'gallery_mosaic');
  const testimonial = s.find((sec: any) => sec?.type === 'testimonial');
    ...
    <>
      ...
      <section className="editorial reveal" id="dining">
        ...
        <div className="editorial-text">
          ...
          {dining?.stats && (dining.stats?.length ?? 0) > 0 && (
