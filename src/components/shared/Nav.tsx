...
export default function Nav({ siteName, siteNameItalic, bookingUrl }: NavProps) {
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-[1002] px-12 transition-all duration-[400ms] ease-in-out flex justify-between items-center ${
        scrolled && !menuOpen
    ...
    >
      ...
      <Link
        ...
        className={`font-heading text-[1.35rem] font-normal tracking-[0.25em] uppercase transition-colors duration-[400ms] ${
          scrolled && !menuOpen ? "text-ocean-deep" : "text-white"
        }`}
      >
        {siteName}{" "}
      ...
      <div className="flex items-center gap-6">
        ...
        <Link
          href={bookingUrl}
          className={`text-[0.65rem] tracking-[0.25em] uppercase font-medium px-7 py-2.5 transition-all duration-300 ${
            menuOpen ? "hidden" : ""
          } ${
            scrolled
